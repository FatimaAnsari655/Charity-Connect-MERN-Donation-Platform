...
export default function Donations() {
  ...
  const refresh = () => {
    setLoading(true);
    fetch("/api/admin/donations").then(r=>r.json()).then(setDonations).finally(()=>setLoading(false));
  };
  ...
  const assignVolunteer = async (id) => {
    const volunteer = prompt("Enter volunteer email or name to assign:");
    if (!volunteer) return;
    await fetch(`/api/admin/donations/${id}/assign`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
              ...
              <tr key={i}>
                ...
                <td>
                  <div className="actions">
                    ...
                    <button onClick={() => assignVolunteer(d._id)}>Assign</button>
                  </div>
                </td>
              </tr>
            ))}
